...
type myDataFile struct {
	...
	wmutex  sync.Mutex   // 写操作需要用到的互斥锁。 多个程序同时修改写操作进度的时候需要用到该互斥锁额
	rmutex  sync.Mutex   // 读操作需要用到的互斥锁。 多个程序同时修改读操作进度的时候需要用到该互斥锁额
...
//todo 注意漏掉某个序列号的情形,比如我想读取第1000序列号的数据，但是由于此时写的比较慢，压根没有这段数据，则下次再读的时候只会到了1001序列号的数据了，当1000序列号写入了数据也没戏了,怎么办，不停的循环直到读到它呗
...
func (df *myDataFile) Read() (rsn int64, d Data, err error) {
	...
	//todo 下面最大的弊端就是需要不停的读锁定读解锁无法使用 defer  df.fmutex.RUnlock()
